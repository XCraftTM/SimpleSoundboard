...
class SoundboardScreen(
    ...
    private var detailLabel: TextWidget? = null
    private var detailLabelY: Int = 0
    ...
    private fun updateDetailLabel(message: Text) {
        // Remove old label if it exists
        detailLabel?.let { remove(it) }

        // Create new label with updated message
        detailLabel = TextWidget(message, textRenderer)
        detailLabel!!.setPosition(width / 2 - detailLabel!!.width / 2, detailLabelY)
        addDrawableChild(detailLabel)
    }
    ...
    fun selectSound(file: File?) {
        ...
        if (file == null) {
            updateDetailLabel(Text.translatable("gui.opensoundboard.select_hint").formatted(Formatting.GRAY))
            ...
            updateDetailLabel(Text.translatable("gui.opensoundboard.settings_for", file.name).formatted(Formatting.YELLOW))
