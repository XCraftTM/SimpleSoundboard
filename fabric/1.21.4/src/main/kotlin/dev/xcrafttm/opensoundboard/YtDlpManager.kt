...
object YtDlpManager {
    ...
    private fun binFile(): File {
        if (!_root_ide_package_.dev.xcrafttm.opensoundboard.OpenSoundboardClient.modDir.exists()) _root_ide_package_.dev.xcrafttm.opensoundboard.OpenSoundboardClient.modDir.mkdirs()
        return File(_root_ide_package_.dev.xcrafttm.opensoundboard.OpenSoundboardClient.modDir, binaryName)
    }

    private fun ffmpegBinFile(): File {
        if (!_root_ide_package_.dev.xcrafttm.opensoundboard.OpenSoundboardClient.modDir.exists()) _root_ide_package_.dev.xcrafttm.opensoundboard.OpenSoundboardClient.modDir.mkdirs()
        return File(_root_ide_package_.dev.xcrafttm.opensoundboard.OpenSoundboardClient.modDir, ffmpegBinaryName)
    }
    ...
    fun ensureFfmpegPresent(): Boolean {
        ...
        return try {
            ...
            val archiveFile = File(_root_ide_package_.dev.xcrafttm.opensoundboard.OpenSoundboardClient.modDir, archiveName)
            downloadBinary(ffmpegDownloadUrl, archiveFile)

            if (isWindows) {
                extractZip(archiveFile, _root_ide_package_.dev.xcrafttm.opensoundboard.OpenSoundboardClient.modDir)
            } else {
                extractTarXz(archiveFile, _root_ide_package_.dev.xcrafttm.opensoundboard.OpenSoundboardClient.modDir)
            }
    ...
    fun downloadUrlIntoSoundDir(
        ...
        val soundDir = _root_ide_package_.dev.xcrafttm.opensoundboard.OpenSoundboardClient.soundDir.also { if (!it.exists()) it.mkdirs() }
